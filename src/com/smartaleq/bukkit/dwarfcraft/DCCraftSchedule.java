...
class DCCraftSchedule implements Runnable {
	...
	@Override
	public void run() {
		// in this task we need to check to see if they are still using a craftbench. if so, continue the task.
		if ( entityPlayer == null ) return;
		if ( entityPlayer.activeContainer == entityPlayer.defaultContainer ) return;
		try { workBench = (ContainerWorkbench)(entityPlayer.activeContainer); } catch (Exception e) { return; }
		...
		if (outputStack != null) {
			...
			for (Skill s:dwarf.getSkills()){
				...
				workBench.b.a(0, outputStack); // set the output stack on the crafting bench
			}
		}
		plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new DCCraftSchedule(plugin, dwarf), 2);
	}
}