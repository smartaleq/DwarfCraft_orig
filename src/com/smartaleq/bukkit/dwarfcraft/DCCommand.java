	...
	public boolean execute(CommandSender sender, String commandLabel, String[] args){
		...
		try {
			if (commandName.equalsIgnoreCase("debug")) {
				...
				if(!sender.isOp()) throw new DCCommandException(plugin, Type.NEEDPERMISSIONS);
				Integer i = 0;
				...
				outputList = parser.parse(desiredArguments, false);
				plugin.getOut().info(sender); return true;
				...
				outputList = parser.parse(desiredArguments, false);
				plugin.getOut().rules(sender); return true;
				...
				plugin.getOut().tutorial(sender, page); return true;
			} 
...
//				outputList = parser.parse(desiredArguments);} 
				...
				if (args.length == 0 && sender instanceof Player) {
					...
					return true;
				} else if (args.length == 0) throw new DCCommandException(plugin, Type.CONSOLECANNOTUSE);
				...
				plugin.getOut().printSkillSheet(dwarf, sender, displayName, printFull);
				return true;
				...
				try{ 
					outputList = parser.parse(desiredArguments, false);
					if (args.length>outputList.size()) throw new DCCommandException(plugin, Type.TOOMANYARGS);
				...
				if(args.length==0 && sender instanceof Player) plugin.getOut().race((Player)sender);
				Dwarf dwarf = new Dwarf(plugin, null);
...
//	}
	...
	private void race(boolean elf, boolean confirm, Dwarf dwarf){
		if(elf){
			if (dwarf.isElf()) plugin.getOut().alreadyElf(sender, dwarf);
			else if (confirm) plugin.getOut().becameElf(sender, dwarf);
			else plugin.getOut().confirmBecomingElf(sender, dwarf);
		}
		else{
